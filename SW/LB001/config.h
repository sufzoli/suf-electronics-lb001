/*
 * config.h
 *
 *  Created on: 2013.12.31.
 *      Author: zoli
 *
 *      This file contains all of the global configuration data
 *      It is collected to a single space to be able to configure the code for various scenarios and hardware
 */

#ifndef CONFIG_H_
#define CONFIG_H_

#define CONFIG_BTN_VECTOR_MAX	10	// Maximum length of the array containing the possible vectors can be called by the buttons
#define CONFIG_BTN_COUNT	4	// Number of the programmable buttons


#endif /* CONFIG_H_ */
